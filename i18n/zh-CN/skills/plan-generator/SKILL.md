---
description: 当我需要为儿童制定或更新 IEP/BIP 方案时，需自动汇总初访、评估和 FBA 数据，结合发展心理学和神经科学视角进行全维度分析，生成包含先备技能链、强化物管理、影子老师撤除和家庭干预计划的完整方案，并更新核心档案。
---
...
2. 用 `obsidian search query="关键词" path="08-Knowledge/教案库" limit=10` 搜索已验证的同类教案
3. 用 `obsidian search query="关键词" path="08-Knowledge/教材" limit=10` 搜索发展里程碑和神经科学相关参考
4. 将检索到的内容融入方案设计，用 `> [!tip] 循证依据：[[概念卡片名]]` 标注
...
| **抗干扰/抑制控制** | 能否在干扰下保持目标行为 | 影响集体环境和泛化 |
...
用 `obsidian create name="Client-[代号] - IEP-{{当前日期}}" content="..." silent` 新建文件（不覆盖旧版）：
- 路径：`01-Clients/Client-[代号]/Client-[代号] - IEP-{{当前日期}}.md`
- 内容：参照下方【输出规范】

## 第六步：同步更新核心档案

1. 先用 `obsidian read file="Client-[代号] - 核心档案"` 读取目标文件，进行变更预览 → 督导确认后执行
2. 用 `obsidian property:set name="status" value="🟠 方案执行中" file="Client-[代号] - 核心档案"` 更新 frontmatter status
3. 用 `obsidian property:set name="last_updated" value="{{当前日期}}" file="Client-[代号] - 核心档案"` 更新 last_updated
4. 用 Edit 工具在 `### 🔗 全生命周期索引` 中追加新 IEP 链接（保留旧链接）
5. 用 Edit 工具更新 `### 📋 当前干预目标索引` 章节（摘要当前 IEP 的核心目标列表）

## 第七步：更新 MOC + 变更日志

1. 用 `obsidian append file="_MOC" content="- [[Client-[代号] - IEP-{{当前日期}}]]"` 在对应个案条目下追加新 IEP 双链
2. 用 `obsidian append file="系统变更日志" content="[{{当前日期时间}}] plan-generator → Write IEP-{{当前日期}}.md + Edit 核心档案.md"` 追加变更日志

可选：执行 `obsidian backlinks file="Client-[代号] - IEP-{{当前日期}}"` 验证双链正确建立
...
**制定日期**：{{当前日期}}
**情报源**：[[Client-[代号] - 初访信息表]] | [[Client-[代号] - 能力评估]] | [[Client-[代号] - FBA 分析]] | [[Client-[代号] - 强化物评估]]
...
| 编号 | 目标描述 | 达标标准 | 教学形式 | 初始辅助 | 辅助退缩计划 | 先备依赖 | 数据采集 | 泛化计划 |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| ST1 | [...] | [...] | [DTT/NET/IT/FCT] | [辅助层级] | [退缩路线] | [依赖哪个底层能力/先备技能] | [频率/正确率/持续时间] | [跨人/跨场景/跨材料] |
| ST2 | [...] | [...] | [...] | [...] | [...] | [...] | [...] | [...] |
...
| 阶段 | 强化程式 | 进入条件 | 预计时长 |
|:---|:---|:---|:---|
| 习得期 | CRF（连续强化） | 教学初期 | [X周] |
| 稳定期 | VR3 / VI30s | 正确率 > 80% 连续 3 天 | [X周] |
| 泛化期 | VR5 / 自然强化为主 | 跨场景正确率 > 70% | 持续 |
...
| 连续 3 个数据点下降 | 立即分析原因（强化物饱和？先备缺失？环境变化？） |
| 达标后连续 3 天维持 | 启动泛化程序 |
...
- → `teacher-guide`：为一线老师生成实操指引（含先备技能教学要点）
- → `reinforcer-tracker`：启动强化物效力追踪
